...
inorder :: T a -> [a]
inorder E = []
inorder (N l x r) = let (l', r') = inorder l ||| inorder r 
                    in l' ++ [x] ++ r'
...
combinar E t2  = t2
combinar t1 E  = t1
combinar t1 t2 = let (t, min) = combinar_aux t1 
                 in (N t min t2)
...
combinar_aux (N l x r) = let (l', min) = combinar_aux l 
                         in ((N l' x r), min)
